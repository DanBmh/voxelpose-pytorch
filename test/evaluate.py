...
def main():
    args = parse_args()
    logger, final_output_dir, tb_log_dir = create_logger(config, args.cfg, "eval_map")
    ...
    with torch.no_grad():
        ...
        if "panoptic" in config.DATASET.TEST_DATASET or "skelda" in config.DATASET.TEST_DATASET:
            ...
            tb.field_names = (
                ["Bone Group"]
                + [f"Actor {i+1}" for i in range(len(actor_pcp))]
                + ["Average"]
            )
            for k, v in bone_person_pcp.items():
                tb.add_row(
                    [k] + [f"{i*100:.1f}" for i in v] + [f"{np.mean(v)*100:.1f}"]
                )
            tb.add_row(
                ["Total"] + [f"{i*100:.1f}" for i in actor_pcp] + [f"{avg_pcp*100:.1f}"]
            )
            print(tb)
