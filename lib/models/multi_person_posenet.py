    ...
    def forward(
        ...
        if self.USE_GT:
            ...
            for i in range(batch_size):
                grid_centers[i, : num_person[i], 3] = torch.tensor(
                    range(num_person[i]), device=device
                )
                ...
                if (
                    ...
                    for i in range(batch_size):
                        if pred[i, n, 0, 3] >= 0:
                            ...
                            weights_3d = meta[0]["joints_3d_vis"][
                                i : i + 1, pred[i, n, 0, 3].long(), :, 0:1
                            ].float()
                            count += 1
                            loss_cord = (
                                loss_cord * (count - 1)
                                + criterion_cord(
                                    single_pose[i : i + 1], targets, True, weights_3d
                                )
                            ) / count
                del single_pose
