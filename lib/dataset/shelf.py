...
class Shelf(JointsDataset):
    ...
    def evaluate(self, preds, recall_threshold=500):
        ...
        for i, fi in enumerate(self.frame_range):
            ...
            pred = np.stack(
                [self.coco2shelf3D(p) for p in copy.deepcopy(pred_coco[:, :, :3])]
            )
    ...
    def coco2shelf3D(coco_pose):
        ...
        ) / 2  # Use middle of shoulder to init
        shelf_pose[13] = coco_pose[0]  # use nose to init

        shelf_pose[13] = shelf_pose[12] + (shelf_pose[13] - shelf_pose[12]) * np.array(
            [0.75, 0.75, 1.5]
        )
